...
models.PosModel = models.PosModel.extend({
    ...
    set_table: function(table, isTransfer) {
        if (!table) { // no table ? go back to the floor plan, see ScreenSelector
            this.set_order(null);   
        } else {     // table ? load the associated orders  ...
            var old_order = this.get_order_list(); // get old_order of transfer table
//            if (this.transfer && old_order.length > 0) {
//                old_order[0].destroy({'reason':'abandon'}); // destroy order of transfer table
//            }
            if (this.transfer) {
                this.transfer.table = table;
                this.transfer = null;
            }
            this.table = table;
            var orders = this.get_order_list();  // list order of new table
            if (orders.length) {
...
//            if (this.transfer) {
//                this.merge_order(old_order, this.get_order());
//                this.transfer = null;
//            }
        }
    },

    merge_order: function(old_order, new_order) {
        var self = this;
        if (old_order.length > 0) {
            var order_lines = old_order[0].get_orderlines();
            for (var i=0; i<order_lines.length; i++){
//                new_order.add_orderline(order_lines[i]);
                var orderline = order_lines[i].clone();
